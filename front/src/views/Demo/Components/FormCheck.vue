  ...
  <div class="form_wapper">
    <span>
      <el-card class="box-card">
        <el-form :model="form" :inline="true" size="mini" :rules="rules">
          <template v-for="(item, index) in form.renterList">
            ...
            >
              <el-input
                v-model="item.idCard"
                @input="inputRenterIdCard(item.idType, index)"
              >
                <el-select
                  slot="prepend"
                  :ref="index + 'idType'"
...
<script>
/* 重复的校验规则:必填 */
const baseRule = [{ required: true, message: '证件号不能为空' }];
...
export default {
  ...
  data () {
    return {
      ...
      inputRules: {
        ...
        TAIWAN_COMPATRIOTS_CERTIFICATES: {
  ...
  methods: {
    changeRenterIdType (data, index) {
      ...
      /* 给表单加上新的校验 */
      this.form.renterList[index].rules = [this.inputRules[data]].concat(
        baseRule
      );
    },
